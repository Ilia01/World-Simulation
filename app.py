from world_sim.ui.web import run
...
# =============================
st.set_page_config(
    page_title="World Simulation – Portfolio Dashboard",
    page_icon="🌍",
    layout="wide",
)
...
def _render_grid_image(
    ...
    lineage_colors: bool = False,
) -> np.ndarray:
    """Render a crisp, high-resolution RGB image for the grid."""
    size = world.size

    # Base colors (in 0..255)
    color_empty = np.array([18, 18, 22], dtype=np.uint8)      # dark background
    color_food = np.array([255, 204, 0], dtype=np.uint8)      # yellow
    default_creature = np.array([56, 255, 146], dtype=np.uint8)  # neon green
    ...
    # Paint creatures (override to stand out)
    ...
    # Rotate to align x as rows, y as cols for typical viewing
    img = np.rot90(img, k=1)

    # Upscale using nearest-neighbor to get chunky, crisp cells
    if cell_px > 1:
...
def _controls_sidebar() -> Dict[str, Any]:
    ...
    lineage_colors = st.sidebar.toggle("Use lineage colors for creatures", value=bool(cfg.get("lineage_colors", False)))
    render_plotly = st.sidebar.toggle("Use Plotly renderer (reduces flicker)", value=bool(cfg.get("render_plotly", True)))
...
def _hero_header() -> None:
    ...
    with right:
        if st.session_state.world:
            ...
            c1.metric("Population", len(world.creatures))
...
def _main_content(config: Dict[str, Any]) -> None:
    ...
    with grid_col:
        ...
        if world is None:
            ...
            if bool(cfg.get("render_plotly", True)):
                ...
                st.image(
                    img,
                    caption=f"Tick {st.session_state.tick}",
    ...
    with info_col:
        ...
        if world and st.session_state.logger:
            ...
            for e in events[::-1]:
                st.write(f"[{e.get('timestamp','')}] Tick {e.get('tick','-')}: {e.get('event','')} — {e.get('details','')}")
        else:
            st.caption("No events yet.")

    st.divider()

    if world and world.creatures:
        st.subheader("Top Creatures")
        top = sorted(world.creatures, key=lambda c: c.energy, reverse=True)[:5]
        rows = [
            {
                "x": c.x,
                "y": c.y,
                ...
                "metabolism": round(c.traits.metabolism, 3),
            }
            for c in top
        ]
